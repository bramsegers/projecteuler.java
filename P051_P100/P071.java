...
public class P071 {
    ...
    public static String solve(long num, long denom, long maxNum) {
        long closestNum = 0;
        long closestDenom = 1;
        for (long i = maxNum; i > 1; i--) {
            long c = num * i / denom;
            if (c * closestDenom >= i * closestNum && i % denom != 0) {
                closestNum = c;
                closestDenom = i;
            }
        }
        return closestNum + "/" + closestDenom;
    }

    public static void main(String[] args) {
        System.out.println(P071.solve(3, 7, 1000000));
    }
}
